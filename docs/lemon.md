...
- [Introduction](#the-lemon-parser-generator)
- [1.0 Table of Contents](#10-table-of-contents)
- [2.0 Security Notes](#20-security-note)
- [3.0 Theory of Operation](#30-theory-of-operation)
  - [3.1 Command Line Options](#31-command-line-options)
  - [3.2 The Parser Interface](#32-the-parser-interface)
    - [3.2.1 Allocating The Parse Object On Stack](#321-allocating-the-parse-object-on-stack)
    - [3.2.2 Interface Summary](#322-interface-summary)
  - [3.3 Differences With YACC and BISON](#33-differences-with-yacc-and-bison)
  - [3.4 Building The "lemon" Or "lemon.exe" Executable](#34-building-the-lemon-or-lemonexe-executable)
- [4.0 Input File Syntax](#40-input-file-syntax)
  - [4.1 Terminals and Nonterminals](#41-terminals-and-nonterminals)
  - [4.2 Grammar Rules](#42-grammar-rules)
  - [4.3 Precedence Rules](#43-precedence-rules)
  - [4.4 Special Directives](#44-special-directives)
- [5.0 Error Processing](#50-error-processing)
- [6.0 History of Lemon](#60-history-of-lemon)
- [7.0 Copyright](#70-copyright)
...
- Parser code generated by lemon &rarr; Robust and secure
- The "lemon.exe" command line tool itself &rarr; Not so much
...
- **-d***directory* Write all output files into *directory*. Normally, output files
  are written into the directory that contains the input grammar file.
- **-D*name*** Define C preprocessor macro *name*. This macro is usable by
  [`%ifdef`](#448-the-if-directive-and-its-friends),
  [`%ifndef`](#448-the-if-directive-and-its-friends), and
  [`%if`](#448-the-if-directive-and-its-friends) lines
  in the grammar file.
- **-E** Run the "%if" preprocessor step only and print the revised grammar file.
- **-g** Do not generate a parser. Instead write the input grammar to standard
...
- **-p** Display all conflicts that are resolved by
  [precedence rules](#43-precedence-rules).
...
it (via the [`%extra_argument`](#445-the-extra_argument-directive) directive),
...
[`%code` directives](#441-the-code-directive), then the parse
...
- Use the [`%name` directive](#4411-the-name-directive) to change
  the "Parse" prefix names of the procedures in the interface.
- Use the [`%token_type` directive](#4424-the-token_type-and-type-directives)
  to define the "ParseTOKENTYPE" type.
- Use the [`%extra_argument` directive](#445-the-extra_argument-directive)
...
of C-code named
"[lemon.c](https://sqlite.org/src/tool/lemon.c)".
...
"VS20*NN* x64 Native Tools Command Prompt" window and enter:
...
specified otherwise using the [`%start_symbol`](#4418-the-start_symbol-directive)
...
using the
[`%left`](#4410-the-left-directive),
[`%right`](#4415-the-right-directive) or
[`%nonassoc`](#4412-the-nonassoc-directive) directives. Terminal symbols
...
- [`%code`](#441-the-code-directive)
- [`%default_destructor`](#442-the-default_destructor-directive)
- [`%default_type`](#443-the-default_type-directive)
- [`%destructor`](#444-the-destructor-directive)
- [`%else`](#448-the-if-directive-and-its-friends)
- [`%endif`](#448-the-if-directive-and-its-friends)
- [`%extra_argument`](#445-the-extra_argument-directive)
- [`%fallback`](#447-the-fallback-directive)
- [`%free`](#4426-the-realloc-free-and-stack_size_limit-directives)
- [`%if`](#448-the-if-directive-and-its-friends)
- [`%ifdef`](#448-the-if-directive-and-its-friends)
- [`%ifndef`](#448-the-if-directive-and-its-friends)
- [`%include`](#449-the-include-directive)
- [`%left`](#4410-the-left-directive)
- [`%name`](#4411-the-name-directive)
- [`%nonassoc`](#4412-the-nonassoc-directive)
- [`%parse_accept`](#4413-the-parse_accept-directive)
- [`%parse_failure`](#4414-the-parse_failure-directive)
- [`%right`](#4415-the-right-directive)
- [`%realloc`](#4426-the-realloc-free-and-stack_size_limit-directives)
- [`%stack_overflow`](#4416-the-stack_overflow-directive)
- [`%stack_size_limit`](#4426-the-realloc-free-and-stack_size_limit-directives)
- [`%stack_size`](#4417-the-stack_size-directive)
- [`%start_symbol`](#4418-the-start_symbol-directive)
- [`%syntax_error`](#4419-the-syntax_error-directive)
- [`%token`](#4420-the-token-directive)
- [`%token_class`](#4421-the-token_class-directive)
- [`%token_destructor`](#4422-the-token_destructor-directive)
- [`%token_prefix`](#4423-the-token_prefix-directive)
- [`%token_type`](#4424-the-token_type-and-type-directives)
- [`%type`](#4424-the-token_type-and-type-directives)
- [`%wildcard`](#4425-the-wildcard-directive)
...
the [`%include`](#449-the-include-directive) directive except that
...
on the [`%destructor`](#444-the-destructor-directive) directive below for
...
[`%type`](#4424-the-token_type-and-type-directives) directive.

#### 4.4.4 The `%destructor` directive

The `%destructor` directive is used to specify a destructor for
a non-terminal symbol.
(See also the [`%token_destructor`](#4422-the-token_destructor-directive)
...
syntax in [SQLite](https://sqlite.org/).
...
Use the [`%code`](#441-the-code-directive) directive to add code to
the end of the generated parser.

#### 4.4.10 The `%left` directive

The `%left` directive is used (along with the
[`%right`](#4415-the-right-directive) and
[`%nonassoc`](#4412-the-nonassoc-directive) directives) to declare
...
[precedence rules](#43-precedence-rules)
or on the [`%left`](#4410-the-left-directive) directive
...
one or more terminal symbols. See the section on
[precedence rules](#43-precedence-rules)
or on the [`%left`](#4410-the-left-directive) directive for additional information.
...
See [Error Processing](#50-error-processing).
...
[`%destructor`](#444-the-destructor-directive) directive above.)
...
the [`%token_type`](#4424-the-token_type-and-type-directives) directive)
...
the maximum allowed parser stack size. If this directive does not exist,
...
[`%parse_failure`](#4414-the-parse_failure-directive) routine
...
[SQLite project](https://sqlite.org/) and the source code
to Lemon has been managed as a part of the
[SQLite source tree](https://sqlite.org/src) in the following
files:
