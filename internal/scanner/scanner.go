...
type Scanner struct {
	// Source buffer
	srcBuf []byte // +1 for sentinel for common case of s.next()
...
func (s *Scanner) scanAction() rune {
	// we know the zero'th rune is '{'; start scanning at the next one
	ch := s.next()
	level := 1
	for level > 0 && ch != EOF {
		if ch == '{' {
...
// scanMantissa is a no-op
func (s *Scanner) scanMantissa(ch rune) rune {
	return ch
}

// scanFraction is a no-op
func (s *Scanner) scanFraction(ch rune) rune {
	return ch
}

// scanExponent is a no-op
